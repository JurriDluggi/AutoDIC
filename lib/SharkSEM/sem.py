...
#
    ...
    def GetGeomLimits(self, index):
        return self.connection.Recv('GetGeomLimits', (sem_conn.ArgType.Int, sem_conn.ArgType.Float, sem_conn.ArgType.Float, sem_conn.ArgType.Float, sem_conn.ArgType.Float), self._CInt(index))
    ...
    def GetBeamCurrent(self):
        return self.connection.RecvFloat('GetBeamCurrent')
    ...
    def SetBeamCurrent(self, beam_current):
        self.connection.Send('SetBeamCurrent', self._CFloat(beam_current))
...
#
    ...
    def PresetEnum(self):
        return self.connection.RecvString('PresetEnum')

    def PresetSet(self, preset):
        self.connection.Send('PresetSet', self._CString(preset))
...
#
    ...
    def StgGetMotorized(self):
        return self.connection.Recv('StgGetMotorized', (sem_conn.ArgType.Int, sem_conn.ArgType.Int, sem_conn.ArgType.Int, sem_conn.ArgType.Int, sem_conn.ArgType.Int))

    def StgGetLimits(self, lim_type):
        return self.connection.Recv('StgGetLimits', (sem_conn.ArgType.Float, sem_conn.ArgType.Float, sem_conn.ArgType.Float, sem_conn.ArgType.Float, sem_conn.ArgType.Float, sem_conn.ArgType.Float, sem_conn.ArgType.Float, sem_conn.ArgType.Float, sem_conn.ArgType.Float, sem_conn.ArgType.Float), self._CInt(lim_type))

    def StgWatchdog(self, timeout):
        self.connection.Send('StgWatchdog', self._CFloat(timeout))
...
#
    ...
    def SMGetPivotPos(self):
        return self.connection.Recv('SMGetPivotPos', (sem_conn.ArgType.Int, sem_conn.ArgType.Float))
...
#
    ...
    def HVSetIndex(self, index, asynch = -1):
        if (asynch == -1):
            self.connection.Send('HVSetIndex', self._CInt(index))
        else:
            self.connection.Send('HVSetIndex', self._CInt(index), self._CInt(asynch))

    def HVSetVoltage(self, voltage, asynch = -1):
        if (asynch == -1):
            self.connection.Send('HVSetVoltage', self._CFloat(voltage))
        else:
            self.connection.Send('HVSetVoltage', self._CFloat(voltage), self._CInt(asynch))
...
#
    ...
    def GISEnumUnits(self):
        return self.connection.RecvString('GISEnumUnits')

    def GISEnumLines(self):
        return self.connection.RecvString('GISEnumLines')

    def GISUnitIsCalib(self, unit_id):
        return self.connection.RecvInt('GISUnitIsCalib', self._CInt(unit_id))

    def GISUnitCalib(self, unit_id):
        return self.connection.RecvInt('GISUnitCalib', self._CInt(unit_id))

    def GISUnitRetrAll(self):
        return self.connection.RecvInt('GISUnitRetrAll')

    def GISLineOutgas(self, line_id):
        return self.connection.RecvInt('GISLineOutgas', self._CInt(line_id))

    def GISLineSetPos(self, line_id, pos):
        return self.connection.RecvInt('GISLineSetPos', self._CInt(line_id), self._CInt(pos))

    def GISLineGetPos(self, line_id):
        return self.connection.RecvInt('GISLineGetPos', self._CInt(line_id))

    def GISLineSetValve(self, line_id, valve):
        return self.connection.RecvInt('GISLineSetValve', self._CInt(line_id), self._CInt(valve))

    def GISLineGetValve(self, line_id):
        return self.connection.RecvInt('GISLineGetValve', self._CInt(line_id))

    def GISLineSetTemp(self, line_id, temperature):
        return self.connection.RecvInt('GISLineSetTemp', self._CInt(line_id), self._CInt(temperature))

    def GISLineGetTemp(self, line_id):
        return self.connection.RecvInt('GISLineGetTemp', self._CInt(line_id))

    def GISLineCloseAll(self):
        return self.connection.RecvString('GISLineCloseAll')
...
#
    ...
    def TI2CRead(self, unit_id, bus_index, address, wr_buffer, wr_bits, rd_bits):
        return self.connection.Recv('TI2CRead', (sem_conn.ArgType.Int, sem_conn.ArgType.ArrayByte), self._CInt(unit_id), self._CInt(bus_index), self._CUnsigned(address), self._CArrayByte(wr_buffer), self._CInt(wr_bits), self._CInt(rd_bits))

    def TI2CWrite(self, unit_id, bus_index, address, wr_buffer, wr_bits):
        return self.connection.Send('TI2CWrite', self._CInt(unit_id), self._CInt(bus_index), self._CUnsigned(address), self._CArrayByte(wr_buffer), self._CInt(wr_bits))

################################################################################
#
# Stitching presets, stitching calibration
#

    def StitPresetEnum(self):
        return self.connection.RecvString('StitPresetEnum')

    def StitPresetSet(self, preset):
        self.connection.Send('StitPresetSet', self._CString(preset))

    def StitSetBulk(self, mag_calibration, aspect_ratio_x, aspect_ratio_y, stitching_rotation):
        self.connection.Send('StitSetBulk', self._CFloat(mag_calibration), self._CFloat(aspect_ratio_x), self._CFloat(aspect_ratio_y), self._CFloat(stitching_rotation))

    def StitGetBulk(self):
        return self.connection.Recv('StitGetBulk', (sem_conn.ArgType.Float, sem_conn.ArgType.Float, sem_conn.ArgType.Float, sem_conn.ArgType.Float))
...
#
    ...
    def DbgScScanMap(self, frameid, width, height, dwell, point_list, single):
        return self.connection.Send('DbgScScanMap',
                    self._CUnsigned(frameid),
                    self._CUnsigned(width),
                    self._CUnsigned(height),
                    self._CUnsigned(dwell),
                    self._CArrayUnsigned(point_list),
                    self._CInt(single))

    def DbgLithoGetMix(self, mixer):
        return self.connection.Recv('DbgLithoGetMix', (sem_conn.ArgType.Float, sem_conn.ArgType.Float, sem_conn.ArgType.Float, sem_conn.ArgType.Float, sem_conn.ArgType.Float, sem_conn.ArgType.Float), self._CInt(mixer))

    def DbgLithoSetMix(self, mixer, a11, a12, q1, a21, a22, q2):
        self.connection.Send('DbgLithoSetMix', self._CInt(mixer), self._CFloat(a11), self._CFloat(a12), self._CFloat(q1), self._CFloat(a21), self._CFloat(a22), self._CFloat(q2))

    def DbgShutterSet(self, DetManId, PosId):
        self.connection.Send('DbgShutterSet', self._CInt(DetManId), self._CInt(PosId))

    def DbgShutterGet(self, DetManId):
        return self.connection.RecvInt('DbgShutterGet', self._CInt(DetManId))

    def DbgAutoGunFine(self, channel):
        self.connection.Send('DbgAutoGunFine', self._CInt(channel))
